...
export function MemberList({
  ...
  async function copyGuestList() {
    const lines = members.map((member, index) => {
      ...
      return `${index + 1}. ${displayName(member)} - ${labels.join(" · ")}`;
    });
    ...
    <>
      {canManage && members.length > 0 ? (
        <div className="mt-4 flex justify-end">
          <button type="button" onClick={() => void copyGuestList()} className="kk-btn-secondary">
            <ClipboardList className="h-4 w-4" />
            {copied ? t("copiedGuestList") : t("copyGuestList")}
          </button>
        </div>
      ) : null}
