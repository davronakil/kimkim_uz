...
7. **Stripe** — webhook endpoint `https://kimkim.uz/api/stripe/webhook` (if paid events)
...
| `STRIPE_SECRET_KEY` | Wrangler secret | Paid events only |
| `STRIPE_WEBHOOK_SECRET` | Wrangler secret | Paid events only |
